...
bandNum = floor(length(freqBands) - 1); % Number of frequency bands
...
workBands = zeros(chanNum, bandNum, subjNum);
restBands = zeros(chanNum, bandNum, subjNum);
...
for subj = subjNum:-1:1
    ...
    for electrode = 1:chanNum
        ...
        % Removing the signal mean
        timeWork = timeWork - mean(timeWork);
        timeRest = timeRest - mean(timeRest);
...
%% SUBJECT BAND POWER OVER LOBES BOXPLOTS
figure
subplot(2, 3, 1)
...
restMSCBand = zeros(chanNum, chanNum, bandNum);
workMSCBand = zeros(chanNum, chanNum, bandNum);

for i = 1:bandNum
    restMSCBand(:,:,i) = mean(restMSCAvg(:, :, freqBands(i)*10+1:freqBands(i+1)*10+1), 3) - eye(chanNum);
    workMSCBand(:,:,i) = mean(workMSCAvg(:, :, freqBands(i)*10+1:freqBands(i+1)*10+1), 3) - eye(chanNum);
end
...
for i = 1:bandNum
    subplot (2, 3, i)
    histogram(nonzeros(squeeze(restMSCBand(:, :, i))), 51)
end

% Histogram on inter-channel coherence in working conditions
figure('name', 'Work Inter-Channel Coherence')
for i = 1:bandNum
    subplot (2, 3, i)
    histogram(nonzeros(squeeze(workMSCBand(:, :, i))), 51)
end

%% INTER-CHANNEL WEAK COHERENCE
cmp = parula(10000);

% Threshold choice based on previous histograms
thr1 = 2000; thr2 = 4000;
...
for k = 1:bandNum
    ...
    for i = 1:chanNum
        for j = 1:chanNum
            ...
            % Checking if the MSC between the two electrodes is in target range
            if i == j || tmp<thr1 || tmp>=thr2
                break
            end
            % Rescaling for plotting color range
            tmp = floor((tmp-thr1)*(10000-1)/(thr2-1-thr1)+1);
            line([-chanlocs(i).Y; -chanlocs(j).Y], [chanlocs(i).X; chanlocs(j).X], 'Color', cmp(tmp,:), 'linewidth', 1.25)
            hold on
        end
    end
    xlabel('Left<--   -->Right'), ylabel('Back<--   -->Front')
    axis square, title(freqLabels(k)), colorbar, clim([thr1 thr2]./10000)
end
...
for k = 1:bandNum
    ...
    for i = 1:chanNum
        for j = 1:chanNum
            % Changing MSC order of magnitude
            tmp = floor(restMSCBand(i,j,k) * 10000) + 1;
             % Checking if the MSC between the two electrodes is in target
             % range
            if i == j || tmp < thr2
                break
            end
            % Rescaling for plotting color range
            tmp = floor((tmp - thr2) * (10000 - 1) / (10000 - thr2) + 1);
            line([-chanlocs(i).Y; -chanlocs(j).Y], [chanlocs(i).X; chanlocs(j).X], 'Color', cmp(tmp,:), 'linewidth', 1.25)
            hold on
        end
    end
    xlabel('Left<--   -->Right'), ylabel('Back<--   -->Front')
    axis square, title(freqLabels(k)), colorbar, clim([thr2 10000]./10000)
end
